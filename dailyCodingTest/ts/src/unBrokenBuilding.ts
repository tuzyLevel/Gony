namespace UnBrokenBuilding {
  export const solution = (
    board: number[][],
    skill: [number, number, number, number, number, number][]
  ) => {
    let result = 0;
    const obj: { [row: number]: { [col: number]: number } } = {};

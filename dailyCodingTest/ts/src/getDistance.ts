namespace GetDistance {
  const checkDistanceRule = (place: string[]) => {
    const offset = [
      [0, 2],
      [0, 1],
      [1, 1],
      ...
      for (let cIdx = 0; cIdx < place[rIdx].length; cIdx++) {
        if (place[rIdx][cIdx] === "P")
          for (const [rOffset, cOffset] of offset) {
            ...
            if (place[rIdx + rOffset][cIdx + cOffset] !== "P") continue;

            if (
              ((rOffset === -1 || rOffset === 1) && cOffset === 0) ||
              (rOffset === 0 && cOffset === 1)
            )
              return false;
  ...
  export function solution(places: string[][]) {
    const result = [];

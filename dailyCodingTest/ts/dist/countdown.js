...
(function (CountDown) {
    ...
    function solution(target) {
        ...
        while (q.length > 0) {
            ...
            if (work.value > 0 && work.count[0] >= minimumCount)
                continue;
            if (work.value >= 50) {
                q.enqueue(new Node(work.value - 50, [work.count[0] + 1, work.count[1] + 1]));
                for (let i = 20; i > 16; i--) {
                    if (work.value - 3 * i >= 0) {
                        q.enqueue(new Node(work.value - 3 * i, [work.count[0] + 1, work.count[1]]));
                        break;
                    }
                }
            }
                ...
                if (work.value % 2 === 0 && +(work.value / 2) <= 20) {
                    q.enqueue(new Node(work.value - Math.floor(work.value / 2) * 2, [
                        work.count[0] + 1,
                        work.count[1],
                    ]));
                }
                else {
                    q.enqueue(new Node(work.value - Math.floor(work.value / 3) * 3, [
                        work.count[0] + 1,
                        work.count[1],
                    ]));
                }

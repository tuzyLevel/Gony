...
(function (NumberCard) {
    ...
    //     return result;
    //   };
    NumberCard.getGCD = (arrayA) => {
        const commonDivisorsOfFirstNumber = [];
        for (let i = 1; i <= arrayA[0]; i++) {
            if (arrayA[0] % i === 0)
                commonDivisorsOfFirstNumber.push(i);
        }
        while (commonDivisorsOfFirstNumber) {
    ...
    NumberCard.solution = (arrayA, arrayB) => {
        const GCD_A = NumberCard.getGCD(arrayA);
        const GCD_B = NumberCard.getGCD(arrayB);

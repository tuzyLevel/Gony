...
(function (Schedule) {
    ...
    Schedule.solution_1 = solution_1;
    function solution_2(n, cores) {
        if (n <= cores.length)
            return n;
        const timeGroup = {};
        ...
        for (const [index, value] of cores.entries()) {
            if (timeGroup[value])
                timeGroup[value].push(index);
            else
                timeGroup[value] = [index];
        }
        ...
        for (let i = 0; i < cores.length; i++) {
            ...
            if (zeroCount === finishedWork - beforeFinishedWork)
                return i + 1;
        }
    }
    Schedule.solution_2 = solution_2;
